...
def get_bundle_root() -> Path:
    ...
    candidates = [
        here.parent.parent,  # if script is in environment/
        Path.cwd(),
        Path.cwd().parent,
    ]
...
def check_filesystem(bundle_root: Path) -> List[CheckResult]:
    expected = {
        ...
        "course_materials/": bundle_root / "course_materials",
        "datasets/": bundle_root / "datasets",
        "environment/": bundle_root / "environment",
        "verification notebook": bundle_root / "environment" / "00_verify_environment.ipynb",
    }
    results: List[CheckResult] = []
...
def main() -> int:
    ...
    print("You should now be able to run the course materials for the summer school.")
    return 0
