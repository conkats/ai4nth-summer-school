...
REM Assumes this script lives in: AI_NTH_Bundle\scripts\
REM ------------------------------------------------------------
...
set "ENV_ARCHIVE=%BUNDLE_DIR%\env\ai-nth-summerschool_windows.zip"
set "ENV_DIR=%BUNDLE_DIR%\portable_env"
set "PYTHON_EXE=%ENV_DIR%\python.exe"
set "CONDA_UNPACK=%ENV_DIR%\Scripts\conda-unpack.exe"
set "VERIFY_NOTEBOOK=%BUNDLE_DIR%\verify\00_verify_environment.ipynb"
set "NOTEBOOKS_DIR=%BUNDLE_DIR%\notebooks"
...
if exist "%VERIFY_NOTEBOOK%" (
    ...
    call "%PYTHON_EXE%" -m jupyter lab --ServerApp.root_dir="%BUNDLE_DIR%" "%NOTEBOOKS_DIR%"
)
